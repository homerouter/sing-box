...
static int build_ipv4_mapped_ipv6_sock_addr_prog(
    ...
    for (size_t i = 0; i < allow_jump_count; ++i) {
        patch_jump(&b, allow_jumps[i], drop_native_ipv6 ? drop_label : allow_label);
    }
    for (size_t i = 0; i < drop_jump_count; ++i) {
        patch_jump(&b, drop_jumps[i], drop_label);
    }
...
int bpf2socks_inbound_prepare(
    const char *cgroup_path,
    uint16_t listen_port,
    struct bpf2socks_bpf_runtime *runtime) {
    if (runtime == NULL || listen_port == 0U) {
        errno = EINVAL;
        return -1;
    }
    ...
    config.token_ipv4_prefix_bits = BPF2SOCKS_TOKEN_IPV4_PREFIX_BITS;
    config.token_ipv6_prefix_bits = BPF2SOCKS_TOKEN_IPV6_PREFIX_BITS;
    if (inet_pton(AF_INET, "127.0.0.0", config.token_ipv4_prefix) != 1 ||
        inet_pton(AF_INET6, "fd7a:7374:6572:6973::", config.token_ipv6_prefix) != 1) {
        errno = EINVAL;
        return -1;
    }
    if (cgroup_path != NULL && cgroup_path[0] != '\0') {
        int written = snprintf(config.cgroup_path, sizeof(config.cgroup_path), "%s", cgroup_path);
        if (written < 0 || (size_t)written >= sizeof(config.cgroup_path)) {
            errno = ENAMETOOLONG;
            return -1;
        }
    }
    ...
    runtime->bypass_socket_cookie_map_fd = create_bypass_socket_cookie_map(BPF2SOCKS_MAX_TOKEN_MAP_ENTRIES);
    if (runtime->token_map_fd < 0 || runtime->udp_peer_map_fd < 0 ||
        runtime->bypass_socket_cookie_map_fd < 0) {
        goto fail;
    }

    runtime->cgroup_fd = open_cgroup_path(config.cgroup_path);
    if (runtime->cgroup_fd < 0 || bpf2socks_detach_named_progs(runtime->cgroup_fd) < 0) {
        goto fail;
    }

    runtime->connect4_prog_fd = build_ipv4_sock_addr_prog(
        &policy, &config, -1, -1, -1, -1, -1, -1, -1,
    ...
    runtime->udp6_v4mapped_recvmsg_prog_fd = build_udp6_recvmsg_prog(
        ...
        "b2s_ur6v4m",
        true);
    if (runtime->connect4_prog_fd < 0 || runtime->udp4_sendmsg_prog_fd < 0 ||
        runtime->udp4_recvmsg_prog_fd < 0 || runtime->connect6_v4mapped_prog_fd < 0 ||
...
int bpf2socks_inbound_attach(struct bpf2socks_bpf_runtime *runtime) {
    ...
    if (bpf2socks_attach_prog(runtime->cgroup_fd, runtime->connect4_prog_fd, BPF_CGROUP_INET4_CONNECT) < 0 ||
        bpf2socks_attach_prog(runtime->cgroup_fd, runtime->udp4_sendmsg_prog_fd, BPF_CGROUP_UDP4_SENDMSG) < 0 ||
        bpf2socks_attach_prog(runtime->cgroup_fd, runtime->udp4_recvmsg_prog_fd, BPF_CGROUP_UDP4_RECVMSG) < 0 ||
        bpf2socks_attach_prog(runtime->cgroup_fd, runtime->connect6_v4mapped_prog_fd, BPF_CGROUP_INET6_CONNECT) < 0 ||
        bpf2socks_attach_prog(runtime->cgroup_fd, runtime->udp6_v4mapped_sendmsg_prog_fd, BPF_CGROUP_UDP6_SENDMSG) < 0 ||
        bpf2socks_attach_prog(runtime->cgroup_fd, runtime->udp6_v4mapped_recvmsg_prog_fd, BPF_CGROUP_UDP6_RECVMSG) < 0) {
        int saved = errno;
        bpf2socks_bpf_stop(runtime);
        errno = saved;
        return -1;
