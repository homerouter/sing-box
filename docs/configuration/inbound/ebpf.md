...
List of 48-bit client source MAC addresses allowed to enter the shared-network
proxy path.

The MAC address is read from the TC Ethernet header and is also available to
`source_mac_address` route and DNS rules.

#### shared_network.exclude_mac_address

List of 48-bit client source MAC addresses to bypass.

Exclude rules take priority over include rules. MAC selection is evaluated
before source CIDR selection. MAC addresses can be randomized or spoofed and
must not be used as authentication.
...
Capacity of the shared-network proxy and bypass flow maps.

Default is `65536`. Accepted values are `1` through `1048576`.

### Shared network behavior

The shared-network policy for a new flow is evaluated in this order:

1. MAC exclude/include policy.
2. Source CIDR exclude/include policy.
3. DHCP bypass.
4. `dns_mode`.
5. Local, private, multicast, and `bypass_rule_set` destination CIDRs.
