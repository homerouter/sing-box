...
允许进入 shared-network 代理路径的 48 位客户端源 MAC 地址列表。

MAC 直接从 TC 以太网头读取，并会传递给 `source_mac_address` 路由和 DNS 规则。

#### shared_network.exclude_mac_address

需要绕过的 48 位客户端源 MAC 地址列表。

exclude 优先于 include，MAC 筛选先于源 CIDR 筛选。MAC 地址可以随机化或伪造，
不能作为身份认证手段。
...
shared-network 代理和绕过流 map 的容量。

默认值为 `65536`，允许范围为 `1` 到 `1048576`。

### 共享网络行为

shared-network 对新流按以下顺序执行策略：

1. MAC exclude/include。
2. 源 CIDR exclude/include。
3. DHCP 绕过。
4. `dns_mode`。
5. 本机、私网、多播和 `bypass_rule_set` 目的 CIDR。
...
IPv6 解析器最多支持八个逐跳、路由、目的选项和认证扩展头。分片流量会在选中的
代理路径上被丢弃，因为后续分片不包含令牌改写所需的传输层端口。
...
启用 Clash API 后，可以通过 `GET /ebpf` 按需读取每个 eBPF 入站的诊断计数。
这些计数只记录内核中的异常路径，例如重定向状态分配、可选缓存更新和 TC 报文改写
失败。sing-box 不会定期扫描 map，也不会记录周期性连接统计。

非零计数表示可能存在降级或丢包。报告问题时应将该结果和启动日志一并提供。
