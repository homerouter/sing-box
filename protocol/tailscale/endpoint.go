...
func (t *Endpoint) start() error {
	...
	if t.network.AutoRedirectOutputMark() != 0 {
		netns.SetControlFunc(control.Append(t.network.SocketProtectFunc(), t.network.AutoRedirectOutputMarkFunc()))
		...
		if t.platformInterface.UsePlatformAutoDetectInterfaceControl() {
			netns.SetControlFunc(control.Append(t.network.SocketProtectFunc(), func(network, address string, conn syscall.RawConn) error {
			...
			netns.SetControlFunc(control.Append(t.network.SocketProtectFunc(), func(string, string, syscall.RawConn) error {
				return nil
			}))
		}
	} else {
		bindFunc := t.network.AutoDetectInterfaceFunc()
		netns.SetControlFunc(control.Append(t.network.SocketProtectFunc(), bindFunc))
		if bindFunc != nil {
...
func (t *Endpoint) listenPacket(ctx context.Context, network string, address string) (nettype.PacketConn, error) {
	listenConfig := net.ListenConfig{
		Control: control.Append(
			control.Append(t.network.SocketProtectFunc(), t.network.AutoDetectInterfaceFunc()),
